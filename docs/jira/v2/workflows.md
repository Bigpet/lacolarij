...
- **entityId** (path): The entity ID of the workflow.
...
`GET /rest/api/2/workflow/{workflowId}/project/{projectId}/issueTypeUsages`
...
`GET /rest/api/2/workflow/{workflowId}/workflowSchemes`
...
- **useApprovalConfiguration** (query): Return the new field `approvalConfiguration` instead of the deprecated status properties for approval configuration.
 ...
 *  `created` Sorts by create time.
 *  `updated` Sorts by update time.
- **scope** (query): The scope of the workflow. Global for company-managed projects and Project for team-managed projects.
- **isActive** (query): Filters active and inactive workflows.
