...
describe('issueService', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(pendingOperationsRepository.add).mockResolvedValue(
      'pending-op-1'
    );
  ...
  describe('updateSummary', () => {
    ...
    it('should throw error when issue not found', async () => {
      ...
      await expect(
        issueService.updateSummary('non-existent', 'New summary')
  ...
  describe('updateDescription', () => {
    ...
    it('should throw error when issue not found', async () => {
      ...
      await expect(
        issueService.updateDescription('non-existent', null)
      ).rejects.toThrow('Issue not found');
    });
  ...
  describe('markSynced', () => {
    ...
    it('should throw error when issue not found', async () => {
      ...
      await expect(
        issueService.markSynced('non-existent', 'v2')
      ).rejects.toThrow('Issue not found');
    });
  });
  ...
  describe('markConflict', () => {
    ...
    it('should throw error when issue not found', async () => {
      ...
      await expect(
        issueService.markConflict('non-existent', remoteIssue)
      ).rejects.toThrow('Issue not found');
    });
  });
  ...
  describe('discardLocalChanges', () => {
    ...
    it('should remove pending operations for the issue', async () => {
      ...
      expect(pendingOperationsRepository.deleteByEntityId).toHaveBeenCalledWith(
        'issue-1'
      );
    });
