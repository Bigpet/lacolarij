  ...
  describe("getIssueDiff", () => {
    ...
    describe("status differences", () => {
      it("should detect status differences", () => {
        const local = createMockIssue({ status: "In Progress", statusCategory: "indeterminate" });
        const remote = createMockIssue({ status: "Done", statusCategory: "done" });
    ...
    describe("multiple differences", () => {
      it("should detect multiple field differences", () => {
        ...
        expect(diffs.map((d) => d.field)).toEqual(expect.arrayContaining(["summary", "status", "priority"]));
      });
    });
