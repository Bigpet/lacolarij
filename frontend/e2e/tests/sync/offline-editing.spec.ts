...
test.describe('Offline Editing', () => {
  ...
  test.describe('Offline Detection', () => {
    ...
    test('should show offline indicator in sync status bar', async ({ page }) => {
      ...
      // Should show offline status in sync bar (target within sync-status to avoid matching offline banner)
      const offlineStatus = page.locator('[data-testid="sync-status"] >> text=Offline');
  ...
  test.describe('Pending Operations', () => {
    ...
    test('should show pending count in UI when offline edits exist', async ({ page }) => {
      ...
      // Check for pending badge (target the badge within sync-status to avoid matching offline banner)
